...
```
core/       pure domain — Decimal maths, market-cap classes, no I/O   ← agents reuse this
ingest/     Excel template generator + validating reader
market/     MarketDataProvider + news + fundamentals providers (yfinance)
llm/        AnalystModel protocol → Groq (free) + Claude (premium)
store/      Postgres schema + repositories (the only place with SQL)
services/   ← THE SEAM: upload, delete, build_dashboard, explore, report, billing
agents/     rule agents (concentration, small-cap, drift) + LLM agents + runner
            agents/briefing/  the LangGraph orchestrator→market→sector hierarchy
...
## The agentic layer (phase 2)

- **Insights** — rule agents flag concentration, small-cap exposure (> 5%), and weekly
  allocation drift on every refresh; they self-heal (resolve a concentration and the
  warning disappears). "Analyze now" adds an LLM read of recent news against your
  holdings. See [`app/agents/README.md`](app/agents/README.md).
- **Explore** — look up any ticker's fundamental ratios plus an analyst's read. Free plan:
  5 fresh analyses/day; cached results are free.
- **Weekly report** — a health score, narrative, and red-flag checklist, generated by the
  cron each Monday and on demand.
- **Briefing** — a **hierarchical agent team** (an orchestrator over per-market agents over
  per-sector specialists, wired with **LangGraph**) reads what moved across your holdings
  and reports, for each development, what happened, what it indicates, and whether it's
  **positive or negative**. Runs weekly (Monday cron) and on demand; see
  [`app/agents/README.md`](app/agents/README.md).
- **Company size** — every holding shows a Large/Mid/Small cap chip.

**LLM tiering.** Free users get **Groq** (Llama 3.3 70B, free tier); premium gets
**Claude** (`claude-opus-4-8`). One `AnalystModel` protocol; the provider is chosen by
plan. With no key set, the rule agents carry the dashboard and the LLM tabs say so.

**Billing toggle.** `STRIPE_ENABLED=false` (default) lets the upgrade button flip your own
plan locally to test the premium path; `=true` swaps it for real Stripe Checkout +
signature-verified webhooks. The local flip is reachable *only* in test mode.

### New environment variables (all optional — sensible defaults)

```
# LLM analyst — set at least GROQ_API_KEY to enable the LLM features
GROQ_API_KEY=gsk_...                         # free tier: console.groq.com
GROQ_MODEL=llama-3.3-70b-versatile
ANTHROPIC_API_KEY=sk-ant-...                 # premium (claude-opus-4-8)
ANALYST_COOLDOWN_MINUTES=30                  # min gap between LLM analyst runs
ANALYSIS_TTL_HOURS=6                         # Explore cache lifetime
FREE_EXPLORE_DAILY_LIMIT=5                   # free-plan Explore analyses/day

# Billing — leave STRIPE_ENABLED=false to test the premium path with no Stripe account
STRIPE_ENABLED=false
STRIPE_SECRET_KEY=sk_...
STRIPE_WEBHOOK_SECRET=whsec_...
STRIPE_PRICE_ID=price_...
PUBLIC_URL=https://your-app.onrender.com     # Stripe return URLs
```
